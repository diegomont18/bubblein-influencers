        ...
        <div key={emp.slug} className="flex items-center gap-2 bg-white/[0.02] border border-white/[0.06] rounded-lg px-3 py-1.5 group/emp">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {(emp as unknown as Record<string,unknown>).postsPerMonth != null && <span title="Estimativa de posts publicados por mes no LinkedIn" className={`text-[8px] font-bold px-1 py-0.5 rounded shrink-0 cursor-help ${Number((emp as unknown as Record<string,unknown>).postsPerMonth) >= 4 ? "text-green-400 bg-green-400/10" : Number((emp as unknown as Record<string,unknown>).postsPerMonth) >= 2 ? "text-yellow-400 bg-yellow-400/10" : "text-red-400 bg-red-400/10"}`}>{Number((emp as unknown as Record<string,unknown>).postsPerMonth)}/mes</span>}
            </div>
