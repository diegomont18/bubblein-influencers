/**
 * Test script: validate curious_coder/linkedin-comment-scraper as a cheaper
 * ($1.20/1k vs $2.00/1k) replacement for harvestapi comments extraction.
 *
 * Run: npx tsx temp/test-curious-coder-comments.ts
...
async function testActor(postUrl: string) {
  console.log(`\n=== curious_coder comments: ${postUrl.slice(0, 80)} ===`);
  const url = `https://api.apify.com/v2/acts/curious_coder~linkedin-comment-scraper/run-sync-get-dataset-items?token=${encodeURIComponent(TOKEN!)}`;
